...
contract CPAN is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function initialize() initializer public {
        __ERC20_init("CryptoPlanes", "CPAN");
        __Pausable_init();
        __Ownable_init();
    ...
    // PANCAKELP ADDRESS
    function setPancakeLPAddress(address addr) public onlyOwner {
        _pancakeLPAddress = addr;
    }
    ...
    function withdraw(address recipient, uint256 amount) external virtual {
        require(_msgSender() != address(0) && _msgSender() == _rewardAddress, "CPAN: Invalid sender");
    ...
    function transferFrom(
        ...
        require(!_isLockTransferForAntiBot || sender == _lpCreatorAddress, "CPAN: Antibot revert");
        if (recipient == _pancakeLPAddress) {
            // SELL
            uint256 _fee = 0;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if (_msgSender() == _pancakeLPAddress) {
            // BUY
            require(!_isLockTransferForAntiBot || recipient == _lpCreatorAddress, "CPAN: Antibot revert");
            uint256 _fee = 0;
            if (recipient != _lpCreatorAddress && recipient != 0x10ED43C718714eb63d5aA57B78B54704E256024E) {
                require(_limiter.isValidBuy(recipient, amount), "CPAN: Buy limit reach");
